...
class sp_worker inherits sp_worker::params {
  ...
  */
  ...
  exec { "stop-server":
    command     => "kill -term $(cat ${install_path}/wso2/${product_profile}/runtime.pid)",
    path        => "/bin/",
    onlyif      => "/usr/bin/test -f ${install_path}/wso2/${product_profile}/runtime.pid",
  ...
  /*
    ...
    Note: Ensure
    that file is available in modules -> sp_worker -> files
  */
